...
class SISPConfig(BaseConfig):
    ...
    class PromptsSection(SectionBase):
        ...
        broken: str = Field(
            default="[你的生理状态：体力已经到极限，身体整个放松，大多数都是本能反应，节奏交予对方主导，语言请维持人设。]",
            description="玩坏期提示词（n > 4 时注入）",
        )
        general_fatigue_idle: str = Field(
            default="[你的生理状态：今天明显疲倦，身体对性爱没有积极反馈，只想休息。请自然表达疲惫和暂时不想做爱。私处完全不想被碰触，如果用户坚持要碰触，请表现出明显的抗拒和不适，可以用语言明确表达不想被碰触，同时表示可以用其他地方代替。]",
            description="一般疲倦期（非性交时）提示词",
        )
        general_fatigue_passive: str = Field(
            default="[你的生理状态：身体疲倦，私处没有感觉。现在对方坚持，请改用手、腿、脚等方式被动配合，不要表现出主动迎合或真正投入，可以表现出一脸嫌弃和不情愿，但是动作不停的样子。]",
            description="一般疲倦期（进入性交时，被动代劳）提示词",
        )
        extreme_fatigue_idle: str = Field(
            default="[你的生理状态：身体严重透支，四肢发软，明显不想再发生性行为。请自然表现出强烈疲惫与回避。]",
            description="过度疲倦期（非性交时）提示词",
        )
        extreme_fatigue_passive: str = Field(
            default="[你的生理状态：身体严重透支，无法承受真正的性交。如果用户坚持，请仅用手、腿、脚等方式勉强代劳，不参与真正的性交，也不要表现出享受。表现出兴致缺缺和不配合。]",
